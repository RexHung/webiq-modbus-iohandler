...
int main() {
  std::string cfg = R"JSON({
    ...
    "items": [
      { "name": "dbl.w", "unit_id": 1, "function": 16, "address": 110, "count": 4, "type": "double", "word_order": "DCBA" },
      { "name": "dbl.r", "unit_id": 1, "function": 3,  "address": 110, "count": 4, "type": "double", "word_order": "DCBA" }
    ]
  })JSON";
  write_text("unit_api_double_word_order_dcba.json", cfg);

  IoHandle h = CreateIoInstance(nullptr, "unit_api_double_word_order_dcba.json");
  assert(h != nullptr);

  int rcw = WriteItem(h, "dbl.w", "-42.5");
  assert(rcw == 0);
  ...
  assert(std::abs(v - (-42.5)) < 1e-9);

  DestroyIoInstance(h);
  std::puts("unit_api_double_word_order_dcba: ok");
  return 0;
}
